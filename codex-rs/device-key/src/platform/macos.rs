...
impl DeviceKeyProvider for MacOsDeviceKeyProvider {
    fn create(&self, request: ProviderCreateRequest<'_>) -> Result<DeviceKeyInfo, DeviceKeyError> {
        ...
        match create_or_load_key_info(
            &secure_enclave_key_id,
    ...
    fn binding(
        ...
        let _ = MacKeyClass::from_protection_class(protection_class)
            .ok_or(DeviceKeyError::KeyNotFound)?;
        load_binding(key_id)
    }
...
fn create_or_load_key_info(
    ...
    let info = key_info(key_id, class, &key)?;
    store_binding(key_id, binding)?;
    Ok(info)
}
