...
impl CodexMessageProcessor {
    ...
    pub(super) async fn plugin_read(
        ...
        let plugin = match read_source {
            Ok(marketplace_path) => {
                ...
                let app_summaries =
                    plugin_app_helpers::load_plugin_app_summaries(&config, &outcome.plugin.apps)
                        .await;
            ...
            Err(remote_marketplace_name) => {
                ...
                let plugin_apps = remote_detail
                    ...
                    .collect::<Vec<_>>();
                let app_summaries =
                    plugin_app_helpers::load_plugin_app_summaries(&config, &plugin_apps).await;
    ...
    async fn remote_plugin_install(
        ...
        if let Err(err) = validate_plugin_segment(&plugin_name, "plugin name") {
            self.send_invalid_request_error(request_id, format!("invalid remote plugin id: {err}"))
                .await;
            return;
        }
