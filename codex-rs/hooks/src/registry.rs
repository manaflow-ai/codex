...
impl Hooks {
    ...
    pub fn preview_subscription_exhausted(
        &self,
        request: &SubscriptionExhaustedRequest,
    ) -> Vec<codex_protocol::protocol::HookRunSummary> {
        self.engine.preview_subscription_exhausted(request)
    }

    pub async fn run_subscription_exhausted(
        &self,
        request: SubscriptionExhaustedRequest,
    ) -> SubscriptionExhaustedOutcome {
        self.engine.run_subscription_exhausted(request).await
    }
}
